...
# Minimal per-edge anchor set for sparse/disconnected graphs (finding #7 fix):
# four corners span both axes, enough to fit a per-axis affine under tiny noise.
SPARSE_ANCHOR_POINTS = ((0.08, 0.08), (0.86, 0.86), (0.08, 0.86), (0.86, 0.08))
TRANSFORMS = {
...
def anchor_specs(mode: str) -> list[tuple[str, tuple[str, ...], float, float]]:
    ...
    # FINDING #7 FIX: sparse/disconnected edges share only a MINIMAL set of anchor
    # points (SPARSE_ANCHOR_POINTS: four corners, enough to fit a per-axis affine),
    # NOT the full 36-point grid. This makes "sparse" sparse by VOLUME, not just by
    # graph topology. Original code reused `grid` here -> 108 items / 216 records,
    # heavier than "complete" (144). New sparse: 3 edges x 4 pts x 2 obs = 24 records.
    specs = []
    for edge_index, edge in enumerate(edges):
        for idx, (x, y) in enumerate(SPARSE_ANCHOR_POINTS):
